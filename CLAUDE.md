...
- **No hardcoded names.** Don't add defaults like a hostname or username. The hostname prompt defaults to the current hostname, and the username is required. The only fixed values the user chose are SSH port **2743** and the timezone default **Asia/Kolkata**.
...
- **Ask once.** Hostname, timezone and admin username are asked only in `bootstrap.sh`; don't add those prompts back to `setup.sh`.
...
- sshd deliberately has **no `ListenAddress`** bound to the Tailscale IP, because sshd can start before `tailscale0` has an address at boot. The Tailscale-only restriction is enforced by the UFW rule `allow in on tailscale0 to any port 2743`. A `sed` strips leftover `ListenAddress` lines from older versions. Do not reintroduce binding.
...
- Tunables are globals at the top of `setup.sh`: `TARGET_PORT=2743`, `LYNIS_TARGET_SCORE=83`, rollback delay.
...
- The SSH port used to be 2626. `configure_ufw_firewall` deletes the old `tailscale0` 2626 rule on reruns.
