...
- **Provider firewall is a prompt, not automation.** `confirm_provider_firewall` (last step, after Lynis) prints the provider-neutral rules, then asks `done/not/skip`. It stores `done|pending|skipped` in `/var/lib/server-setup/provider-firewall`, which `check-health.sh` reads. It isn't asked again once `done` or `skipped`. Don't call provider APIs.
- **Public repo.** Never commit keys, tokens, enrollment keys or real server IPs/hostnames; `.gitignore` blocks `*.pem`, `*.key` and `id_*`.
- **Admin VPN is a prompt.** `prompt_vpn_engine` asks tailscale (default) or wireguard; `configure_vpn` dispatches to `install_tailscale` or `configure_wireguard`, sets `VPN_INTERFACE`/`VPN_ADMIN_IP` (used by the SSH gate, UFW rule, printed commands and summary) and records the answer in `/var/lib/server-setup/vpn-engine` for `check-health.sh`. Everything downstream must stay interface-agnostic: no hardcoded `tailscale0` outside the Docker rules block, whose VPN RETURN line is rewritten with `sed` to `$VPN_INTERFACE`. The CrowdSec whitelist and ban cleanup cover the WireGuard subnet instead of the Tailscale ranges when WireGuard is chosen.
- **WireGuard path specifics** (`configure_wireguard`, tunables `WG_INTERFACE`/`WG_PORT`/`WG_SUBNET` at the top of `setup.sh`): keys under `/etc/wireguard` with `umask 077`; reruns never overwrite `wg0.conf` (hand-added peers survive) and `wg_add_peer_if_missing` adds the admin peer only when absent; the client config is printed once with a QR code; UFW opens `$WG_PORT/udp` publicly. **The handshake wait is the lockout guard** (WireGuard has no Tailscale SSH fallback): it waits 2 minutes for a peer handshake and, if none arrives, requires an explicit `y` or exits before SSH is touched. Keep that check.
- **Full tunnel is optional and prompted** (`VPN_FULL_TUNNEL`, asked in `prompt_vpn_engine`; state in `/var/lib/server-setup/vpn-full-tunnel`). WireGuard path: `enable_ip_forwarding` writes `/etc/sysctl.d/61-vpn-forward.conf`, `detect_egress_interface` reads the default route, `configure_wireguard_nat` appends a marker-delimited `*nat` block to `/etc/ufw/before.rules`, and `configure_ufw_firewall` adds `ufw route allow in on $WG_INTERFACE out on $VPN_EGRESS_INTERFACE`; the client gets `AllowedIPs = 0.0.0.0/0`. **The NAT block must never declare `:POSTROUTING`**: that flushes the chain on every `ufw reload` and breaks Docker's NAT. IPv6 is intentionally not routed (no v6 subnet on wg0), so don't add `::/0` to the client without also configuring v6 NAT. Tailscale path: `tailscale set --advertise-exit-node`, which still needs console approval.
- **Container engine is a prompt.** `prompt_container_engine` asks docker (default) or podman; `configure_container_engine` dispatches to `configure_docker` or `configure_podman` and records the answer in `/var/lib/server-setup/container-engine`, which `check-health.sh` reads. Keep both paths working: the Docker origin is added to unattended-upgrades and checked by `verify_automatic_updates` only when Docker is chosen, and `configure_docker_firewall` runs only on the Docker path (rootless Podman needs no ufw-docker rules).
...
- Tailscale must be up (`tailscale up`, 2-minute wait) before SSH is touched; the script exits on failure. After Phase 2, Tailscale is the only SSH path, so:
  ...
  - First login: `tailscale_login_with_auth_key` optionally reads a `tskey-` auth key (hidden) and passes it only as `--auth-key=file:<root-only mktemp file>`, which is shredded after use and by an EXIT trap. **Never put the key in argv:** the auditd `execve euid=0` rule would log it. On an empty or rejected key it returns 1 and the script falls back to the browser `tailscale up`.
  - `warn_if_tailscale_tagged`: tagged devices don't match the default Tailscale SSH rule (`dst autogroup:self`), so warn that the emergency path needs a tag-based ssh rule.
...
- Podman (`configure_podman`, rootless): installs `podman podman-docker podman-compose uidmap passt dbus-user-session`, runs `loginctl enable-linger` for the admin user, and enables `podman.socket` + `podman-auto-update.timer` in that user's systemd instance via `runuser … env XDG_RUNTIME_DIR=/run/user/<uid> DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/<uid>/bus systemctl --user`. Podman has no global default publish address, so the script tells the user to write `-p 127.0.0.1:…`, and warns against `sudo podman` (rootful ports bypass UFW).
